...
//        bottom();
//        translate([0, sizeTop[1], 44-.7+0]) rotate([180, 0, 0]) top();
        translate([0, sizeTop[1]+11, 0]) rotate([0, 0, 0]) top();
...
module top() {
    ...
    difference() {
        ...
        // hinge holes
        translate([sizeTop[0]/2, sizeBot[1]+5, 17.6]) rotate([90, 0, 0]) cylinder($fn=32, h=20, d=3.3);
        translate([sizeTop[0]/2, sizeBot[1]-2, 17.6]) rotate([90, 0, 0]) cylinder($fn=6, h=10, d=6.6);
    }
    
    translate([sizeTop[0]/2+8, -5.5, 23]) rotate([0, -90, 0]) hinge_top();
...
module bottom() {   
    ...
    difference() {
        union() {
            ...
            // hinge nut support
            translate([(sizeBot[0]/2)-18, sizeBot[1]-4.8, 1.2]) cube([36, 4.8, 16]);
        }
        ...
        translate([(sizeBot[0]/2)-13, sizeBot[1]+1, 11.5]) rotate([90, 0, 0]) cylinder($fn=32, d=3.3, h=10);
        translate([(sizeBot[0]/2)-13, sizeBot[1]-3.5, 11.5]) rotate([90, 0, 0]) cylinder($fn=6, d=6.6, h=8);
        translate([(sizeBot[0]/2)+13, sizeBot[1]+1, 11.5]) rotate([90, 0, 0]) cylinder($fn=32, d=3.3, h=10);
        translate([(sizeBot[0]/2)+13, sizeBot[1]-3.5, 11.5]) rotate([90, 0, 0]) cylinder($fn=6, d=6.6, h=8);
...
//        translate([153-3, sizeBot[1]-14, -1]) rotate([90, 0, 90]) linear_extrude(height=3) polygon(p_reliefholes);
    ...
    // hinges
//    translate([sizeBot[0]/2+8, sizeBot[1]+5.5, 20]) rotate([0, 90, 180]) hinge_bottom(screwed=true);
