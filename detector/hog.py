...
def run(image):
    ...
    scores = [x[0] for x in weights.tolist()]
    converter.convert_to_json(image_foldername, image_filename, image, imagesize, toMinMaxBoundingBox(rects), classes, scores, output_filename)

    # for (x, y, w, h) in rects:
    #     cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
    #
    # img = resize(img, width=min(1200, img.shape[1]))
    #
